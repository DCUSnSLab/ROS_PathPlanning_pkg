...
public:
    PathPlannerNode() : Node("global_path_planner_node")
    {
        ...
        this->declare_parameter<std::string>("map_file_path", 
            "/home/ros2/ros2_ws/src/scv_robot_gazebo/maps/20250115_k-city.json");
...
private:
    void loadMapData()
    {
        ...
        if (!map_client_->wait_for_service(std::chrono::seconds(5))) {
            RCLCPP_ERROR(this->get_logger(), "Map service not available after 5 seconds");
            return;
        }
    ...
    void buildGraph()
    {
        ...
        // Note: This assumes links represent connections between sequential nodes
        // In a real scenario, you would parse actual link data with FromNodeID and ToNodeID
        
        // For demonstration, create connections between adjacent nodes and some random connections
        for (size_t i = 0; i < map_nodes_.poses.size() - 1; ++i) {
            int from_id = static_cast<int>(i);
            int to_id = static_cast<int>(i + 1);
            
            double distance = calculateDistance(map_nodes_.poses[i], map_nodes_.poses[to_id]);
        ...
        // Add some additional connections for more realistic graph
        for (size_t i = 0; i < map_nodes_.poses.size(); ++i) {
            for (size_t j = i + 2; j < std::min(i + 5, map_nodes_.poses.size()); ++j) {
                int from_id = static_cast<int>(i);
                int to_id = static_cast<int>(j);
                
                double distance = calculateDistance(map_nodes_.poses[i], map_nodes_.poses[j]);
                
                // Only add connection if distance is reasonable (not too far)
                if (distance < 100.0) {  // Adjust threshold as needed
                    adjacency_list_[from_id].emplace_back(to_id, from_id, distance);
                    adjacency_list_[to_id].emplace_back(from_id, to_id, distance);
                }
            }
        ...
        RCLCPP_INFO(this->get_logger(), "Graph built with %zu nodes", node_map_.size());
    }
    
    void publishVisualizationData()
    {
        // Publish nodes and links for visualization
        if (!map_nodes_.poses.empty()) {
            map_nodes_.header.stamp = this->get_clock()->now();
            nodes_publisher_->publish(map_nodes_);
        }
        
        if (!map_links_.poses.empty()) {
            map_links_.header.stamp = this->get_clock()->now();
            links_publisher_->publish(map_links_);
        }
    ...
    void goalCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Goal received: x=%.2f, y=%.2f", 
    ...
    void planPathFromGpsToGoal()
    {
        ...
        // Goal is already in map frame (UTM-like coordinates)
    ...
    // GPS to UTM conversion (same as in gps_odom_publisher)
    void gpsToUTM(double lat, double lon, double& easting, double& northing)
    {
        // Simplified UTM conversion for Zone 52N (Korea)
        const double a = 6378137.0;           // WGS84 semi-major axis
        const double f = 1.0 / 298.257223563; // WGS84 flattening
        const double k0 = 0.9996;            // UTM scale factor
        const double lon0 = 129.0 * M_PI / 180.0; // Central meridian for Zone 52 (129°E)
        const double false_easting = 500000.0;
        const double false_northing = 0.0;   // Northern hemisphere
        
        // Convert to radians
        double lat_rad = lat * M_PI / 180.0;
        double lon_rad = lon * M_PI / 180.0;
        
        // Calculate UTM coordinates using simplified formulas
        double e2 = 2.0 * f - f * f;
        ...
        easting = false_easting + k0 * N * (A + (1.0 - T + C) * A*A*A / 6.0
                                           + (5.0 - 18.0*T + T*T + 72.0*C - 58.0*e_prime_sq) * A*A*A*A*A / 120.0);
        
        northing = false_northing + k0 * (M + N * std::tan(lat_rad) * (A*A / 2.0
